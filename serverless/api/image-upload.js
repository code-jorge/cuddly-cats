import { getThumbnailName, resizeImage, uploadImage } from '../utils/upload'
...
export default async (request) => {
  try {
    ...
    const thumbnailBuffer = await resizeImage(buffer)
    const thumbnailName = getThumbnailName(imageName)
    await uploadImage(thumbnailBuffer, thumbnailName)
    // Return success response
    return Response.json({  message: "Image uploaded successfully.", imageName, thumbnailName })
  } catch (error) {
